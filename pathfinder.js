...
- Never write more than 5 lines total.`;
  ...
  const pfGlowSpinner = document.createElement('div');
  pfGlowSpinner.className = 'pf-glow-spinner';
  pfGlowRing.appendChild(pfGlowSpinner);
  document.body.appendChild(pfGlowRing);

  function syncGlowRing(){
    const r = pfWindow.getBoundingClientRect();
    pfGlowRing.style.top = (r.top-2)+'px'; pfGlowRing.style.left = (r.left-2)+'px';
    pfGlowRing.style.width = (r.width+4)+'px'; pfGlowRing.style.height = (r.height+4)+'px';
  }
