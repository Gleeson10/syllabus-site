...
- Never write more than 5 lines total.`;
  ...
  // Glow ring — canvas orbiting light effect
  const pfGlowCanvas = document.createElement('canvas');
  pfGlowCanvas.style.cssText = 'position:fixed;pointer-events:none;z-index:997;display:none;';
  document.body.appendChild(pfGlowCanvas);
  const pfGlowCtx = pfGlowCanvas.getContext('2d');
  let pfGlowOffset = 0, pfGlowRAF = null, pfGlowVisible = false;

  function syncGlowRing(){
    const r = pfWindow.getBoundingClientRect();
    const pad = 16;
    pfGlowCanvas.style.left = (r.left - pad) + 'px';
    pfGlowCanvas.style.top = (r.top - pad) + 'px';
    const cw = Math.round(r.width + pad * 2), ch = Math.round(r.height + pad * 2);
    if(pfGlowCanvas.width !== cw) pfGlowCanvas.width = cw;
    if(pfGlowCanvas.height !== ch) pfGlowCanvas.height = ch;
  }

  function pfRoundedRectPath(ctx, x, y, w, h, r){
    ctx.beginPath();
    ctx.moveTo(x+r, y);
    ctx.lineTo(x+w-r, y);
  ...
  function drawGlowFrame(){
    if(!pfGlowVisible) return;
    const ctx = pfGlowCtx, pad = 16;
    const cw = pfGlowCanvas.width, ch = pfGlowCanvas.height;
    const bw = cw - pad*2, bh = ch - pad*2, br = 20;
    const perimeter = 2*(bw+bh) + br*(Math.PI*2 - 8);
    const dashLen = perimeter * 0.15;
    pfGlowOffset = (pfGlowOffset - 7 + perimeter) % perimeter;
    ctx.clearRect(0, 0, cw, ch);
    // Outer soft glow
    ctx.save();
    ctx.strokeStyle = 'rgba(123,111,255,0.45)';
    ctx.lineWidth = 7;
    ctx.shadowBlur = 22;
    ctx.shadowColor = '#7B6FFF';
    ctx.setLineDash([dashLen, perimeter - dashLen]);
    ctx.lineDashOffset = pfGlowOffset;
    pfRoundedRectPath(ctx, pad, pad, bw, bh, br);
    ctx.stroke();
    ctx.restore();
    // Inner bright core
    ctx.save();
    ...
    ctx.setLineDash([dashLen * 0.55, perimeter - dashLen * 0.55]);
    ctx.lineDashOffset = pfGlowOffset + dashLen * 0.22;
    pfRoundedRectPath(ctx, pad, pad, bw, bh, br);
    ctx.stroke();
    ctx.restore();
    pfGlowRAF = requestAnimationFrame(drawGlowFrame);
  }
  ...
  pfBtn.addEventListener('click',()=>{
    ...
    if(pfOpen){
      setTimeout(()=>pfInput.focus(),300);
      setTimeout(()=>{syncGlowRing();pfGlowCanvas.style.display='block';pfGlowVisible=true;if(!pfGlowRAF)pfGlowRAF=requestAnimationFrame(drawGlowFrame);},300);
    } else {
      pfGlowVisible=false;pfGlowCanvas.style.display='none';if(pfGlowRAF){cancelAnimationFrame(pfGlowRAF);pfGlowRAF=null;}
    }
  });
  pfClose.addEventListener('click',()=>{pfOpen=false;pfWindow.classList.remove('open');pfGlowVisible=false;pfGlowCanvas.style.display='none';if(pfGlowRAF){cancelAnimationFrame(pfGlowRAF);pfGlowRAF=null;}});
